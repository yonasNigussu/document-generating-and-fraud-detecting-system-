...
<body id="page-top">
  ...
  @include('it_school.sidebar')
    
    <!-- TopBar -->
    @include('it_school.navbar')
        <!-- Topbar -->
        <!-- Container Fluid-->
    ...
    <?php $request_tl = 'To Whom It May Concern'; ?>
    <?php $request = $lists->request_type; ?>
    <?php $department = 'information technology'; ?>
    <?php $request_department = $lists->department; ?>
    @if($request == $request_tl)
     @if($department == $request_department)
        <tr>
            
            <td>{{$lists->first_name}} {{$lists->last_name}}</td>
            <th>{{$lists->department}}<</th>
            <td>{{$lists->request_type}}</td>
            <td>2011/04/25</td>
            <td>
                @if ($lists->status == 'Rejected')
                    <span class="badge badge-danger">{{$lists->status}}</span>
                @elseif ($lists->status == 'Finished')
                    <span class="badge badge-success">{{$lists->status}}</span>
                @else
                    <span class="badge badge-warning">{{$lists->status}}</span>
                @endif
            </td>
            <td><a href="{{URL::to('/detailit',['id' => $lists->id])}}" class="btn btn-sm btn-primary">Detail</a></td>
        </tr>
    @endif
    @endif
@endforeach
